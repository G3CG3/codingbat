package warmup2;

/**
 * Last2 - CodingBat Warmup-2
 *
 * Given a string, return the count of the number of times that a substring length 2
 * appears in the string and also as the last 2 chars of the string.
 * We won't count the end substring itself.
 */
public class Last2 {

    /**
     * Counts how many times the last 2 chars appear in the rest of the string.
     *
     * @param str input string
     * @return count of occurrences
     */
    public int last2(String str) {
        int count = 0;
        if (str.length() < 2) return 0;

        String end = str.substring(str.length() - 2);

        for (int i = 0; i < str.length() - 2; i++) {
            if (str.substring(i, i + 2).equals(end)) {
                count++;
            }
        }
        return count;
    }

    // Optional: main() for testing
    public static void main(String[] args) {
        Last2 l2 = new Last2();
        System.out.println(l2.last2("hixxxhi"));   // 1
        System.out.println(l2.last2("xaxxaxaxx")); // 1
        System.out.println(l2.last2("axxxa"));     // 0
        System.out.println(l2.last2("Hi"));        // 0
    }
}
